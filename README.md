...
for German parcel and freight shipping. Stage 1 builds the calculation core only: input
validation, canonical dimensions, volumetric weight and billable weight. Prices, zones,
shipment grouping and the OXID module wiring come later and are deliberately absent.
...
|---|---|---|
| `Domain` | Units and vocabulary: `VolumetricDivisor`, `PieceOutcome` (`Shippable`, `Rejected`), `ZoneLookup` (`KnownZone`, `UnknownZone`) | `ShippingClass` only |
| `Input` | The request as received: `QuoteRequest`, `OrderLine`, `TariffConfig`, `InputNormalizer` | `Domain` |
| `Validation` | Shop policy — what input is accepted: `InputValidator`, `InputLimits`, `ValidationError`, `ValidationErrorCode` | `Input` |
| `Measurement` | Geometry and weights: `Dimensions`, `VolumetricWeight`, `MeasuredPiece`, `PieceFactory` | `Domain`, and `Input\OrderLine` only |
| `Result` | What leaves the engine: `QuoteResult`, `Quote`, `ValidationFailed`, `InputSnapshot` | `Measurement`, `Validation` |
...
2. Valid lines are expanded into pieces: one `MeasuredPiece` per unit of `quantity`.
3. Per piece: sides are sorted descending into `Dimensions` (canonical order, so girth and
...
4. A `Quote` is returned with the pieces, a normalised `InputSnapshot` and the tariff
   `configVersion`.
...
| Postal code shape | 2 to 10 characters; first must be a letter or digit, the rest may also be spaces or hyphens | `Validation\InputLimits::POSTAL_CODE_PATTERN` |
| Country shape | two letters A–Z after normalisation | `Validation\InputLimits::COUNTRY_PATTERN` |
...
- A malformed country or postal code is broken input, never a "we do not deliver there"
  decision. `CH` passes the shape gate; refusing a destination is a later business rule.
...
- `InputNormalizer` trims the postal code and trims plus upper-cases the country. It does not
...
volumetric weight, billable weight, the normalised input snapshot, and the `ShippingClass`
vocabulary with its monotonic `atLeast()` escalation.

Not included, by decision rather than by omission: prices and totals, resolving a postal code
to a shipping zone, grouping pieces into shipments, surcharges, carrier APIs, persistence,
HTTP and the OXID module integration. The `Domain` outcome and zone types
(`Shippable`, `Rejected`, `KnownZone`, `UnknownZone`) exist as vocabulary for those steps and
are not yet used by `QuoteEngine`.
